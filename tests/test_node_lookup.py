...
class NodeaddToRingTest(unittest.TestCase):
    ...
    def test_ThreeNodesRing(self):
        ...
        Two nodes added by two nodes
        It provides a dict for addToRing method
        Check if successor attr is the proper one
        """
        # add node1 from node0
        self.node0.addToRing({"ip": self.node1.ip, "port": self.node1.port})
        # add node2 from node1
        self.node1.addToRing({"ip": self.node2.ip, "port": self.node2.port})
